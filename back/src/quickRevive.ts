...
export function getQuickRevive(session: TimerUserSession): any {
    if (!session.quickRevive || typeof session.quickRevive !== "object")
        session.quickRevive = { nonce: 0, phase: "idle", startedAt: 0, endsAt: 0, goal: 0, points: 0, resultAt: 0 };
    return session.quickRevive;
}

// is the clock running right now? this is what mysterybox.ts checks before letting a box open.
...
// start the clock. refused while one is already running or a box is being opened — the source is one frame.
export function startQuickRevive(session: TimerUserSession): { ok: boolean, message: string } {
    ...
    qr.phase = "running";
    qr.startedAt = now;
    qr.endsAt = now + cfg.seconds * 1000;
    qr.goal = cfg.points;
    qr.points = 0;
    qr.resultAt = 0;
    emitTerminal(session.userId, `QUICK REVIVE — chat has ${cfg.seconds}s to put up ${cfg.points} sub point${cfg.points === 1 ? "" : "s"}.`, true);
    pushQuickRevive(session);
    if (cfg.announce)
        chatAnnounce(session, `${cfg.title}! Chat has ${cfg.seconds} seconds to hit ${cfg.points} sub point${cfg.points === 1 ? "" : "s"} — Tier 1 = 1, Tier 2 = 2, Tier 3 = 6. Go!`, "blue");
    ...
    t.end = setTimeout(() => {
        try {
            ...
            reportError(session.userId, "ending a quick revive", err);
            endQuickRevive(session);
        }
    }, cfg.seconds * 1000);
    return { ok: true, message: `Quick revive started — ${cfg.seconds}s for ${cfg.points} sub point${cfg.points === 1 ? "" : "s"}.` };
}
...
export function creditQuickRevive(session: TimerUserSession, event: TimerEvent){
    ...
    if (!pts)
        return;
    // the deadline is checked here as well as by the timer, so a sub arriving in the same tick the clock ran
    // out can't be counted after the fact
...
// the clock stops, one way or the other. the sound and the words are the source's job; this just says which.
function decide(session: TimerUserSession, won: boolean){
    ...
    qr.resultAt = now;
    pushQuickRevive(session);
    const tally = `${qr.points} of ${qr.goal} sub point${qr.goal === 1 ? "" : "s"}`;
    if (won){
        emitTerminal(session.userId, `QUICK REVIVE — ${cfg.winText} chat hit ${tally} with ${spare}s to spare.`, true);
        if (cfg.announce)
            chatAnnounce(session, `${cfg.winText} Chat hit ${tally} with ${spare} second${spare === 1 ? "" : "s"} to spare!`, "green");
    } else {
        emitTerminal(session.userId, `QUICK REVIVE — ${cfg.failText} chat got ${tally}.`);
        if (cfg.announce)
            chatAnnounce(session, `${cfg.failText} Chat got ${tally} before the clock ran out.`, "orange");
    }
    clearTimeout(t.done);
    t.done = setTimeout(() => {
        try {
            ...
            reportError(session.userId, "clearing a finished quick revive", err);
        }
    }, cfg.holdSec * 1000);
}
...
export function endQuickRevive(session: TimerUserSession){
    ...
    qr.resultAt = 0;
    if (wasRunning)
        emitTerminal(session.userId, "QUICK REVIVE — called off.", true);
    pushQuickRevive(session);
}
